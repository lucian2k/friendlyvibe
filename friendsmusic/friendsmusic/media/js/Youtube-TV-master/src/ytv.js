    ...
    var YTV = YTV || function(id, opts){
        ...
        var noop = function(){},
            ...
            utils = {
                ...
                ajax: {
                    get: function(url, fn, fn_start){
                        ...
                        if (fn_start != undefined) { fn_start.call(); }
                ...
                endpoints: {
                    ...
                    userVids: function(extra_params){
                        var query_params = false;
                        var root_url = false;

                        if (settings.localapivideosurl) {
                            root_url = settings.localapivideosurl
                        }
                        else {
                            root_url = utils.endpoints.base+'feeds/api/users/'+settings.user+'/uploads/?v=2&alt=json&format=5&max-results=50';
                        }

                        if (extra_params != undefined) {
                            query_params = jQuery.param(extra_params);
                        }
                        if (query_params) {
                            return root_url + '?' + query_params;
                        }
            ...
            prepare = {
                ...
                compileList: function(data, update_only) {
                    if (data && data.results) {
                        var list = '',
                            user = {
                                title: 'FriendlyVibe.com',
                                url: '',
                                thumb: '',
                                summary: 'Generated by FriendlyVibe',
                                subscribers: 0,
                                views: 0
                            },
                            videos = data.results,
                            first = true,
                            last_item = data.update_item,
                            i;
                        ...
                        if (!update_only) { 
                            ...
                            if (data.last_update_human) {
                                list += '<span class="label label-success" id="notice-last-updated">Last updated: ' + data.last_update_human + '</span> <span class="glyphicon glyphicon-cloud-download pull-right"></span>';
                            }
                            else if (data.results.length == 0) {
                                list += '<span class="label label-success" id="notice-last-updated">The playlist is being updated ...</span>';
                            }
                            list += '</a>';
                            list += '</div>';
                            ...
                            first = false;
                            jQuery('#notice-last-updated').html('Last updated: '+data.last_update_human);
                        }
                        ...
                        for(i=0; i<videos.length; i++){
                            if(videos[i].code){
                                ...
                                var viewsCount = 'N/A'
                                if (video.stats) {
                                    viewsCount = utils.addCommas(video.stats.viewCount);
                                }
                                ...
                                // list += '<div class="ytv-content"><b>'+(video.title)+'</b><span class="ytv-views">'+viewsCount+' Views</span></div>';
                        ...
                        // just update the videos list by prepending the new videos
                        if (update_only && (videos.length > 0)) {
                            jQuery(settings.element).find('div.ytv-list-inner').prepend(list);
                        }

                        if(settings.autoupdateinterval){
                            setTimeout(function() {
                                action.endpoints.playlistUpdate(last_item);
                            }, 60000);
                        }
                    }
            ...
            action = {
                ...
                endpoints: {
                    ...
                    playlistUpdate: function(last_item_update) {
                        // updating the playlist's contents with the latest content
                        utils.ajax.get(utils.endpoints.userVids({'lu': last_item_update}),
                            function(data) {
                                prepare.compileList(data, true);
                            },
                            function (e) {
            ...
            initialize = function(id, opts){
                ...
                if(settings.element && (settings.user || settings.localapivideosurl)) {
